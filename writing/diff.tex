...
%DIF DEL draft_old.tex   Sun Oct  9 20:40:13 2022
%DIF ADD draft.tex       Sun Oct  9 20:32:54 2022
...
\begin{document}
	...
	\if0\blind
	{
		\title{\bf \DIFaddbegin \DIFadd{Concurrent }\DIFaddend Data Assimilation for Modeling and Calibrating of Disease-altered Potassium Channel Kinetics in Mouse Cardiomyocytes}
		...
		The Pennsylvania State University, \DIFdelbegin \DIFdel{State College}\DIFdelend \DIFaddbegin \DIFadd{University Park}\DIFaddend , PA USA \\
...
\begin{abstract}
\DIFdelbegin \DIFdel{Activities of cardiac voltage-gated potassium }\DIFdelend \DIFaddbegin \DIFadd{Potassium }\DIFaddend channels (K\textsubscript{v}) are \DIFdelbegin \DIFdel{essential in the electrical conduction system of the heart. Even slight changes in K\textsubscript{v} functions can cause fatal heart disease, such as long QT syndrome. Our recent studies found that altered glycosylation causes aberrant electrical signaling in mouse cardiomyocytesand leads to heart failure and early death. Activities of K\textsubscript{v} channels were also affected by altered glycosylation, and K\textsuperscript{+} currents were significantly reduced. Among major cardiac voltage-gated ion channels (VGIC), K\textsubscript{v} channels have a distinctive feature: various isoforms generate their own unique currents contributing to different }\DIFdelend \DIFaddbegin \DIFadd{responsible for repolarizing the action potential in cardiomyocytes. Different K\textsubscript{v} isoforms and their currents contribute to various }\DIFaddend phases of repolarization\DIFdelbegin \DIFdel{of the cardiac action potential (AP). On the other hand, the dominant isoform is responsible for Na\textsuperscript{+} and Ca\textsuperscript{2+} currents . This characteristic raises challenges for studying K\textsubscript{v}channels. Although it is highly desirable to observe the activities of each K\textsubscript{v} isoform separately, only collective outputs can be measured because of their slightly overlapping voltage-dependence of gating. In whole-cell recordings, the sum of }\DIFdelend \DIFaddbegin \DIFadd{. In the mouse ventricle, repolarization roughly consists of two kinetic components: the rapidly inactivating current I\textsubscript{Kto} (}\DIFaddend K\DIFdelbegin \DIFdel{\textsuperscript{+} }\DIFdelend \DIFaddbegin \DIFadd{\textsubscript{v}4.2), and the slowly inactivating currents I\textsubscript{Kslow1} and I\textsubscript{Kslow2} (K\textsubscript{v}1.5 and K\textsubscript{v}2.1). Data assimilation is used to estimate information on K\textsubscript{v} isoforms, because only the sum of their activities is measured in the form of }\DIFaddend currents (I\textsubscript{\DIFdelbegin \DIFdel{Ksum}\DIFdelend \DIFaddbegin \DIFadd{K}\DIFaddend })\DIFdelbegin \DIFdel{can be measured. Traditional methods apply curve fitting to estimate components of K\textsubscript{v} isoforms from I}\DIFdelend \DIFaddbegin \DIFadd{. Most existing studies separate K\textsuperscript{+} currents mathematically from I}\DIFaddend \textsubscript{\DIFdelbegin \DIFdel{Ksum}\DIFdelend \DIFaddbegin \DIFadd{K}\DIFaddend } \DIFdelbegin \DIFdel{. Additional curve fitting is used to find kinetics information for the individual K\textsuperscript{+} currents}\DIFdelend \DIFaddbegin \DIFadd{recordings and make inference of kinetics by a curve-fitting decomposition procedure}\DIFaddend . However, this traditional approach \DIFdelbegin \DIFdel{is limited in its ability to determine gating kinetics }\DIFdelend \DIFaddbegin \DIFadd{has limitations to scrutinize the kinetics and dynamics }\DIFaddend of K\textsubscript{v} \DIFdelbegin \DIFdel{isoforms rigorously. Curve fittings do not account for interactions of multiple gating processes of various isoformsand dynamics according to changes in protocols. Therefore, we }\DIFdelend \DIFaddbegin \DIFadd{in two-fold: 1) the curve-fitting decomposition only relies on the shape of K\textsuperscript{+} currents without considering underlying kinetics and interactions of K\textsubscript{v} isoforms, and 2) it handles I\textsubscript{K} traces from the same cell one-by-one individually. There is an urgent need to develop new methods that simultaneously model the underlying kinetics of multiple I\textsubscript{K} traces for a better understanding of pathological mechanisms. We }\DIFaddend propose a novel \DIFdelbegin \DIFdel{data assimilation framework for investigating whole-cell K\textsubscript{v} current traces and apply the proposed method to our glycosylation data. First, }\DIFdelend \DIFaddbegin \textit{\DIFadd{concurrent data assimilation}} \DIFadd{framework that utilizes biophysics-based }\DIFaddend computer models of K\DIFdelbegin \DIFdel{\textsuperscript{+} currents are designed with parameters that control the kinetic rates and, in turn, determine the currents. Second, we develop fractional factorial designs to identify the parameters that have significant impacts on the current shapes. Third, we develop model calibration as nonlinear optimization that couple the }\textit{\DIFdel{in-silico}} %DIFAUXCMD
\DIFdel{K}\DIFdelend \textsubscript{v} \DIFdelbegin \DIFdel{models and }\textit{\DIFdel{in-vitro}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{to delineate gating kinetics in the multiple observations concurrently to cope with cellular-level dynamics. We apply the proposed method to glyco-altered }\DIFaddend I\textsubscript{K} recordings \DIFdelbegin \DIFdel{. Last, low-dimensional embedding is used to visualize distributional difference of kinetic parameters of cells, showing the cellular-level variability. }\DIFdelend \DIFaddbegin \DIFadd{from our previous studies. }\DIFaddend Experimental results show that the \DIFdelbegin \DIFdel{data assimilation method can estimate differences in kinetic rate }\DIFdelend \DIFaddbegin \DIFadd{proposed method can discriminate characteristics }\DIFaddend between the control and \DIFdelbegin \DIFdel{experiment }\DIFdelend \DIFaddbegin \DIFadd{experimental }\DIFaddend groups. This \DIFaddbegin \DIFadd{proposed }\DIFaddend method has strong potential to pave a new way to analyze K\textsubscript{v} and heart diseases.
\end{abstract}

\noindent%
{\it Keywords:} Data assimilation, \DIFdelbegin \DIFdel{model calibration}\DIFdelend \DIFaddbegin \DIFadd{computational electrophysiology}\DIFaddend , potassium channels, kinetics modeling, glycosylation

%\newpage
\spacingset{1.5} % DON'T change the spacing!

\section{Introduction}
\DIFdelbegin \DIFdel{Cardiac voltage-gated }\DIFdelend \DIFaddbegin \DIFadd{Voltage-gated }\DIFaddend potassium channels (K\textsubscript{v}) are \DIFaddbegin \DIFadd{critical to the electrical conduction system of the heart. K\textsubscript{v} are }\DIFaddend responsible for repolarizing the \DIFaddbegin \DIFadd{cardiac }\DIFaddend action potential (AP)\DIFdelbegin \DIFdel{in cardiomyocytes. They are indispensable in the electrical conduction system of the heart. Even }\DIFdelend \DIFaddbegin \DIFadd{, a change of membrane potential over time, representing the net electrical activity in a myocyte. The AP is produced by the orchestrated activities of ion channels. Voltage-gated sodium channels (Na\textsubscript{v}) initiates fast depolarization that drives the sharp upstroke of the AP. K\textsubscript{v} play antagonistic roles that repolarize the AP with the rapid inactivation of Na\textsubscript{v}. Repolarization consists of multiple phases, to which different K\textsubscript{v} isoforms contribute with fluxes of Ca\textsuperscript{2+} and Cl\textsuperscript{-} involved through designated ion channels. This complex process can be affected by even }\DIFaddend modest changes in K\textsubscript{v} activities\DIFaddbegin \DIFadd{. Aberrant repolarization }\DIFaddend can significantly affect the AP duration and the QT interval, which lead to fatal heart diseases \citep{ravens2008role}. Glycosylation is a co/posttranslational modification that is critical for protein functions including activities of K\textsubscript{v} and other voltage-gated ion channels (VGICs) \citep{ohtsubo2006glycosylation,ednie2012modulation}. A growing number of studies in diverse directions, ranging from genome-wise to proteomic/glycomic searches, have shown clear links between altered protein glycosylation and heart diseases \citep{yung2004gene,yang2015glycoproteins,miura2016glycomics,nagai2016aberrant}. Despite its importance, the underlying mechanism is still largely unknown how changes in glycosylation contribute to heart disease onset and progression. Our \textit{in-vitro} experiments showed that glycosylation regulations modulate VGIC activities and contribute to both electrical and contractile dysfunction \citep{ednie2013sialicNav1,ednie2015sialicKv,ednie2019reduced}. In addition, we verified our experimental results via \textit{in-silico} studies and estimate how changes in the lower level (VGICs) affect the functions in the higher level (cardiomyocytes), which is one of the powers of systematic computer models \citep{du2013silico,du2015statistical,du2017silico,kim2022simulation}. \textbf{Figure~\ref{fig:framework_diagram}} illustrates this feedback loop of studying heart diseases, which will be referred to as the data assimilation framework that combines experimental data with mathematical/computational models.
\begin{figure}[!ht]
    \centering
...
However, a distinctive feature of K\textsubscript{v} raises challenges in both \textit{in-vitro} and \textit{in-silico} studies: K\textsubscript{v} have \DIFdelbegin \DIFdel{diverse isoforms , with each of them showing unique biophysical properties and }\DIFdelend \DIFaddbegin \DIFadd{various isoforms }\DIFaddend playing different roles in \DIFdelbegin \DIFdel{the }\DIFdelend AP repolarization \citep{nerbonne2005molecular}\DIFaddbegin \DIFadd{, whereas dominant isoforms exist for other major VGICs \mbox{%DIFAUXCMD
\citep{abriel2010cardiac,benitah2010type}}\hskip0pt%DIFAUXCMD
. In the mouse ventricle, repolarization is roughly delineated as two kinetic components: the rapidly inactivating current I\textsubscript{Kto}, conducted through K\textsubscript{v}4.2, and the slowly inactivating currents such as I\textsubscript{Kslow1} and I\textsubscript{Kslow2}, conducted through K\textsubscript{v}1.5 and K\textsubscript{v}2.1 respectively}\DIFaddend . \DIFdelbegin \DIFdel{In contrast, there is a predominant isoform for Na\textsubscript{v}and Ca\textsubscript{v}\mbox{%DIFAUXCMD
\citep{abriel2010cardiac,benitah2010type}}\hskip0pt%DIFAUXCMD
. }\DIFdelend Although it is highly desirable to \DIFdelbegin \DIFdel{observe the activities of each K\textsubscript{v} isoform separately, only collective outputs }\DIFdelend \DIFaddbegin \DIFadd{study individual K\textsubscript{v} isoforms to understand pathological mechanisms in the cardiac conduction system, only the collective activities as a whole }\DIFaddend can be measured \DIFaddbegin \DIFadd{in the form of currents using whole-cell voltage-clamp K\textsubscript{v} currents (I\textsubscript{K}) }\DIFaddend because of their slightly overlapping voltage-dependence of gating \citep{brouillette2004functional}. \DIFdelbegin \DIFdel{Specifically, only the sum of K\textsuperscript{+} currents (I\textsubscript{K}) can be recorded in whole-cell voltage-clamp experiments. Typically, a form of curve fitting is }\DIFdelend \DIFaddbegin \DIFadd{A data assimilation method, combining experimental data with mathematical/computational models, can }\DIFaddend used to estimate \DIFaddbegin \DIFadd{information on K\textsubscript{v} isoforms. Most existing studies separate }\DIFaddend individual K\textsuperscript{+} currents \DIFdelbegin \DIFdel{, in which parameters of the functional form of the currents are searched, thereby the sum of them best describing I}\DIFdelend \DIFaddbegin \DIFadd{mathematically from I}\DIFaddend \textsubscript{K} \DIFdelbegin \DIFdel{recordings \mbox{%DIFAUXCMD
\citep{brunet2004heterogeneous}}\hskip0pt%DIFAUXCMD
. Then the shape parameters are used to calibrate computer models \mbox{%DIFAUXCMD
\citep{kim2022simulation}}\hskip0pt%DIFAUXCMD
, or kinetics information is added to the data assimilation process with additional curve fitting if applicable \mbox{%DIFAUXCMD
\citep{du2017silico}}\hskip0pt%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{by a curve-fitting decomposition procedure, assuming a curve of I\textsubscript{K} consists of the summation of exponential decay functions and a constant term; each represents the shape of K\textsuperscript{+} currents \mbox{%DIFAUXCMD
\citep{brunet2004heterogeneous}}\hskip0pt%DIFAUXCMD
. }\DIFaddend However, this traditional approach is limited in its ability to \DIFdelbegin \DIFdel{rigorously determine kinetics and gating }\DIFdelend \DIFaddbegin \DIFadd{scrutinize the kinetics and dynamics }\DIFaddend of K\textsubscript{v} \DIFdelbegin \DIFdel{isoforms in two aspects}\DIFdelend \DIFaddbegin \DIFadd{in two-fold}\DIFaddend : 1) \DIFdelbegin \DIFdel{This two-step curve-fitting method does not consider the dynamics of }\DIFdelend the \DIFaddbegin \DIFadd{curve-fitting I\textsubscript{K} decomposition only relies on the shape of K\textsuperscript{+} current without considering }\DIFaddend underlying kinetics and \DIFdelbegin \DIFdel{current outputs by separating the two processes working together and simplifying them with curve functions}\DIFdelend \DIFaddbegin \DIFadd{interactions of K\textsubscript{v} isoforms}\DIFaddend , and 2) it \DIFdelbegin \DIFdel{does not account for the cellular-level dynamics according to changes in protocols, as one current trace is analyzed separately}\DIFdelend \DIFaddbegin \DIFadd{handles I\textsubscript{K} traces from the same cell with different conditions (i.e., clamp voltages) one-by-one individually}\DIFaddend . These limitations not only hinder \textit{in-silico} modeling results but, in some cases, make it impossible if modeling relies on kinetic data estimated from experimental data \citep{kim2022simulation}. \DIFdelbegin \DIFdel{Therefore, there }\DIFdelend \DIFaddbegin \DIFadd{There }\DIFaddend is an urgent need to develop new data assimilation methods \DIFdelbegin \DIFdel{and tools that delineate the kinetics of K}\DIFdelend \DIFaddbegin \DIFadd{that simultaneously delineate the underlying kinetics of multiple I}\DIFaddend \textsubscript{\DIFdelbegin \DIFdel{v}\DIFdelend \DIFaddbegin \DIFadd{K}\DIFaddend } \DIFdelbegin \DIFdel{isoforms for better understanding their roles in heart diseases}\DIFdelend \DIFaddbegin \DIFadd{traces for a better understanding of the ion channels to provide insights into clinical applications, such as drug design}\DIFaddend . 

\DIFdelbegin \DIFdel{This paper presents a novel approach to data assimilation for analyzing whole-cell K\textsubscript{v} current traces (I\textsubscript{K}) and modeling the underlying }\DIFdelend \DIFaddbegin \DIFadd{Therefore, this paper proposes a new approach of }\textit{\DIFadd{concurrent data assimilation}} \DIFadd{for investigating the }\DIFaddend kinetics of K\textsubscript{v} isoforms \DIFaddbegin \DIFadd{from multiple I\textsubscript{K} traces measured in the same cell}\DIFaddend . A schematic flowchart of the proposed framework is shown in \textbf{Figure~\ref{fig:flow_chart}B} and compared with the current approach in \textbf{Figure~\ref{fig:flow_chart}A}. First, \DIFdelbegin \DIFdel{we designed biophysical computer models that simulate activities of ion channels to replace simple current-shape functions in curve fitting. Computer models of ion channels are a system of differential equations consisting of gating variables and kinetic functions that continuously simulate ion channel activities over time. Parameters in kinetic functions control the gating of ion channels }\DIFdelend \DIFaddbegin \textit{\DIFadd{in-silico}} \DIFadd{models of K\textsubscript{v} and their currents are designed with parameters that control the kinetic rates }\DIFaddend and, in turn, determine \DIFdelbegin \DIFdel{current outputs. Second, }\DIFdelend \DIFaddbegin \DIFadd{the currents. It is worth mentioning that K\textsuperscript{+} currents are generated based on the mathematically simulated gating mechanism as opposed to the curve-fitting procedure, which only mimics the shape of the currents. Second, we perform }\DIFaddend a sensitivity analysis \DIFdelbegin \DIFdel{was performed to screen out parameters that marginally influence the model outputs. Third, we developed a model calibration that minimizes discrepancy between model outputs and experimental data generated from multiple protocols. Last, we made estimations and inferences of impacts of disease-related perturbations on potassium channel kinetics and used }\DIFdelend \DIFaddbegin \DIFadd{of the kinetic parameters using fractional factorial designs to identify the parameters that have significant impacts on the current generation. Several markers are defined that best represent the characteristics of the currents in various perspectives. Third, }\textit{\DIFadd{in-vitro}} \DIFadd{data of I\textsubscript{K} recordings are coupled with the proposed models by calibrating the parameters selected by the sensitivity analysis. We apply the proposed framework to glyco-altered IK recordings from our previous studies. Our data showed that reduced N-glycosylation in mouse cardiomyocytes is sufficient to cause dilated cardiomyopathy (DCM), which is the third most common cause of heart failure and the major reason for heart transplantation \mbox{%DIFAUXCMD
\citep{weintraub2017dilated}}\hskip0pt%DIFAUXCMD
. Last, }\DIFaddend low-dimensional embedding \DIFdelbegin \DIFdel{to show cellular variability}\DIFdelend \DIFaddbegin \DIFadd{is utilized to visually inspect the differences of the calibrated parameters in a collective way across the cells}\DIFaddend . Experimental results \DIFdelbegin \DIFdel{of }\DIFdelend \DIFaddbegin \DIFadd{show that }\DIFaddend the proposed method \DIFdelbegin \DIFdel{applied to our glycosylation data provide kinetics information and differences between the healthy control and the disease group}\DIFdelend \DIFaddbegin \DIFadd{provides reliable estimations of K\textsubscript{v} isoform kinetics of both the wild type and disease-altered groups and successfully captures cellular variability}\DIFaddend .
\begin{figure}
    \centering
...
The main advantage of this approach is that it analyzes \DIFaddbegin \DIFadd{multiple }\DIFaddend I\textsubscript{K} recordings \DIFaddbegin \DIFadd{concurrently }\DIFaddend based on the biophysical \DIFdelbegin \DIFdel{models with considering cellular level dynamics according to changes in protocols for }\DIFdelend \DIFaddbegin \DIFadd{computer models, providing strong potential for modeling the kinetics and gating mechanism of the cardiac ion channels for }\DIFaddend studying heart diseases. Our contributions are summarized as follows:
\begin{itemize}
    \item We modeled the underlying kinetics of K\textsubscript{v} isoforms from whole-cell I\textsubscript{K} recordings based on biophysical principles rather than curve-fitting \DIFdelbegin \DIFdel{methods}\DIFdelend \DIFaddbegin \DIFadd{procedures}\DIFaddend .
    \item We developed a \DIFdelbegin \DIFdel{model calibration }\DIFdelend \DIFaddbegin \DIFadd{concurrent data assimilation }\DIFaddend routine that harnesses multiple protocols simultaneously rather than a single protocol independently \DIFaddbegin \DIFadd{one-by-one}\DIFaddend , resulting in comprehensive cellular-level modeling.
    \item The proposed approach \DIFdelbegin \DIFdel{builds }\DIFdelend \DIFaddbegin \DIFadd{generates }\DIFaddend separate K\textsubscript{v} models and \DIFdelbegin \DIFdel{calibrates them }\DIFdelend \DIFaddbegin \DIFadd{visualize the distributions of the kinetic parameters }\DIFaddend for each cell, which allows analyzing the cellular variability. 
...
\end{itemize}
The proposed data assimilation framework has strong potential to serve as a novel complementary data \DIFdelbegin \DIFdel{analytics tool to in-vitro }\DIFdelend \DIFaddbegin \DIFadd{analytical tool to }\textit{\DIFadd{in-vitro}} \DIFaddend experiments. 
...
\subsection{\DIFdelbegin \DIFdel{Data }\DIFdelend \DIFaddbegin \DIFadd{Concurrent }\DIFaddend Assimilation \DIFdelbegin \DIFdel{and Model Calibration}\DIFdelend \DIFaddbegin \DIFadd{of Functional Data}\DIFaddend }
\DIFdelbegin \textit{\DIFdel{Data assimilation}} %DIFAUXCMD
\DIFdel{is a method }\DIFdelend \DIFaddbegin \DIFadd{Data assimilation is a systematic procedure }\DIFaddend to find the optimal configuration and state of computational\DIFaddbegin \DIFadd{/mathematical }\DIFaddend models by coupling \DIFdelbegin \DIFdel{them }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend with experimental data. Experimental data $\mathcal{D}$ are observations of a real process $\mathcal{R}$ that represents scientific phenomena under investigation. The output of physical experiments $y^{\mathcal{D}}(x)$, given input $x$, inevitably contains errors for various reasons, such as noise in measurement or experimental environment. Suppose $\mathcal{D}$ and $\mathcal{R}$ can be related as follows in Equation~\ref{eq:rd_relation}, where $\epsilon$ is the error term. 
\begin{equation}
...
\DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{Traditionally, bi-/tri-exponential functions as described in Equation~\ref{eq:expl_fitting} have served as $y^{\mathcal{M}}(x|\theta)$, where $\theta=\{\mathrm{A}_{i}(x),\tau_{i}(x)\}$. Note that the model parameters $\mathrm{A}_{i}(x)$ (amplitude) and $\tau_{i}(x)$ (time constant) depend on input data $x$, which are clamp voltages for whole-cell I\textsubscript{K} recordings. In general, multiple input protocols are applied to a myocyte producing a set of I\textsubscript{K} recordings to study the voltage-dependent characteristics of VGICs. However, these exponential functions cannot analyze simultaneously, leading to potential information loss in the cellular-level dynamics of ion channels. In contrast, computer models of VGICs can generate currents given one set of parameters because they are designed by biophysical principles encoding voltage dependence. In }\DIFaddend this study, $\delta$ is defined by the \DIFdelbegin \DIFdel{sum }\DIFdelend \DIFaddbegin \DIFadd{summation }\DIFaddend of root-mean-square errors (RMSEs) as \DIFaddbegin \DIFadd{given }\DIFaddend in Equation~\ref{eq:rmse}\DIFdelbegin \DIFdel{, which measures deviations between experimental data and model predictions }\DIFdelend \DIFaddbegin \DIFadd{. RMSE evaluates the goodness-of-fit between a I\textsubscript{K} recording recording and model prediction }\DIFaddend from the end of a holding potential \DIFdelbegin \DIFdel{$t_i^{(h)}$ }\DIFdelend \DIFaddbegin \DIFadd{$t_v^{(h)}$ }\DIFaddend to the end of voltage step \DIFdelbegin \DIFdel{$t_i^{(e)}$ across a range of protocols, $i=1,2,\cdots,n$.
}\DIFdelend \DIFaddbegin \DIFadd{$t_v^{(e)}$. RMSEs from the same cell are summed up to guide the optimization procedure calibrating the models in a concurrent way for clamp voltage $v=1,2,\cdots,n$.
}\DIFaddend \begin{equation}
    \label{eq:rmse}
    \delta = \sum\DIFdelbegin \DIFdel{_{i=1}}\DIFdelend \DIFaddbegin \DIFadd{_{v=1}}\DIFaddend ^{n} \DIFdelbegin \DIFdel{\sqrt{\int_{t_i^{(h)}}^{t_i^{(e)}}\frac{(y_i^{\mathcal{D}}(t) - y_i^{\mathcal{M}}(t|\theta))^2}{t_i^{(e)}-t_i^{(h)}}dt}
}\DIFdelend \DIFaddbegin \DIFadd{\sqrt{\int_{t_v^{(h)}}^{t_v^{(e)}}\frac{(y_v^{\mathcal{D}}(t) - y_v^{\mathcal{M}}(t|\theta))^2}{t_v^{(e)}-t_v^{(h)}}dt}
}\DIFaddend \end{equation}
\DIFaddbegin 

\DIFaddend This approach differs from previous studies in two-fold. First, unlike the traditional curve fitting using the exponential model described in Equation~\ref{eq:expl_fitting}, the suggested method includes multiple protocols simultaneously for comprehensive cellular-level modeling. Second, it calibrates computer models directly to I\textsubscript{Ksum} recordings, while the previous studies use statistics estimated from the data \citep{du2015statistical, du2017silico, kim2022simulation}. Calibrating to the current traces themselves raises a challenge in optimization but has advantages when it is hard to estimate statistics from data reliably \citep{kim2022simulation}. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend We developed the box-constrained nonlinear optimization routine using nonlinear optimization algorithms with multi-random initial points to minimize $\delta$. Box constraints mean that $\theta$ has a lower and upper bound for each dimension, so solution space is constrained in a hypercube. In this way, the optimization loop can be controlled by users, allowing them to blend their domain knowledge into the modeling. Note that it is possible because the suggested method is based on biophysical models that provide interpretability of $\theta$ in channel kinetics rather than the curve fitting. Global optimum is not guaranteed due to the complexity of our models. We developed the multi-random-start scheme to avoid local optima and find the solution as close to the optimum as possible. Latin hypercube designs and parallel computations are used to sample initial points and run them on multicores to compensate for the increased computational burden. This work is implemented in MATLAB R2022a, and the code is available \DIFdelbegin \DIFdel{at xyz}\DIFdelend \DIFaddbegin \DIFadd{as online supplemental material}\DIFaddend .
...
\DIFdelFL{Kinetics functions and their relationships to voltage of I\textsubscript{Kslow1} (A) steady-state activation, (B) steady-state inactivation, (C) activation time constant, (D) inactivation time constant, (E) I\textsubscript{Kslow2} inactivation time constant, and (F) I\textsubscript{Kss} activation time constant.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:kinetics_rec}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Parameter Distributions Between WT and MGAT1KO}} %DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{s:results.distribution}
%DIFDELCMD < %%%
\DIFdel{We estimated distributions of the calibrated kinetic parameters using histograms to analyze the impacts of reduced N-glycosylation further. }\textbf{\DIFdel{Figure~\ref{fig:hist_pkto}}} %DIFAUXCMD
\DIFdel{shows the distributional differences of the kinetic parameters in the K\textsubscript{4.2} model between the two group. A notable fact about the figure is that MGAT1KO shows small variances for Parameters 1, }\DIFdelend \DIFaddbegin \DIFadd{It is worth mentioning that the proposed method extrapolates $\tau_{i}^{(2)}$ that is greater than }\DIFaddend 5 \DIFdelbegin \DIFdel{, and 7 in transition $\alpha_{a}$ and $\beta_{a}$, which control kinetics activation of I\textsubscript{Kto}. Reduced glycosylation affects activation gating that plays a vital role in I\textsubscript{Kto}, as it is characterized by a high peak at the early depolarization and rapid inactivation. In addition, MGAT1KO causes a leftward shift in the distribution of the maximum conductance $G_{\mathrm{Kto}}$, resulting in smaller conduction than WT in general. It is likely due to the observed current reductions in MGAT1KO}\DIFdelend \DIFaddbegin \DIFadd{s from the 4.5-s-pulse protocol}\DIFaddend .
\begin{figure}[!ht]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \includegraphics{figs/hist_pkto.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics{figs/kinetics_rec.pdf}
    \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Distribution estimations of calibration parameters in the K\textsubscript{Kto} model.}%DIFDELCMD < \MBLOCKRIGHTBRACE
%DIFDELCMD <     \label{fig:hist_pkto}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Figure~\ref{fig:hist_prectifiers}}} %DIFAUXCMD
\DIFdel{shows distribution estimations of the calibration parameters in the rectifier and steady-state currents models. }\textbf{\DIFdel{Figure~\ref{fig:hist_prectifiers}A}} %DIFAUXCMD
\DIFdel{shows significant distributional differences in Parameters 9 }\DIFdelend \DIFaddbegin \DIFadd{Kinetics functions }\DIFaddend and \DIFdelbegin \DIFdel{10, which control time constants of I}\DIFdelend \DIFaddbegin \DIFadd{their relationships to voltage of I}\DIFaddend \textsubscript{Kslow1} \DIFdelbegin \DIFdel{. It is compatible with the previous observations of significant differences in time constants to voltage relationships of I\textsubscript{Kslow1} between WT and MGAT1KO. There are slight shifts in Parameters 1 and 4 of I\textsubscript{Kslow1} that affect }\DIFdelend \DIFaddbegin \DIFadd{(A) }\DIFaddend steady-state \DIFdelbegin \DIFdel{activation. The voltage-threshold parameter for MGAT1KO in }\DIFdelend \DIFaddbegin \DIFadd{activation, (B) }\DIFaddend steady-state \DIFdelbegin \DIFdel{inactivation has higher densities at low values around 30 than WT, which has the peak mode around 48. It accounts for the shift at low voltages less than 0 mV in SSI. The maximum conductance $G_{\mathrm{Kslow1}}$ of MGAT1KO is centered around relatively small values compared to WT. This leftward shift in the maximum conductance is also observed in I}\DIFdelend \DIFaddbegin \DIFadd{inactivation, (C) activation time constant, (D) inactivation time constant, (E) I}\DIFaddend \textsubscript{Kslow2} \DIFaddbegin \DIFadd{inactivation time constant, and }\DIFaddend (\DIFdelbegin \textbf{\DIFdel{Figure~\ref{fig:kinetics_rec}B}}%DIFAUXCMD
\DIFdel{). Although there is a difference in variance of $G_{\mathrm{Kss}}$, the two groups have similar means around 0.03 (i.e. WT: $0.0375\pm0.0027$ vs MGAT1KO: $0.0325\pm0.015$). It is compatible with the observation that I}\DIFdelend \DIFaddbegin \DIFadd{F) I}\DIFaddend \textsubscript{Kss} \DIFdelbegin \DIFdel{does not show significant reductions. 
}%DIFDELCMD < \begin{figure}[!ht]
%DIFDELCMD <     \centering
%DIFDELCMD <     \includegraphics{figs/hist_prectifiers.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Distribution estimations of calibration parameters in (A) the I\textsubscript{Kslow1} model, (B) the I\textsubscript{Kslow2} model, and (C) the I\textsubscript{Kss} model.}}
    %DIFAUXCMD
%DIFDELCMD < \label{fig:hist_prectifiers}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \DIFaddFL{activation time constant.}}
    \label{fig:kinetics_rec}
\DIFaddendFL \end{figure}

\DIFdelbegin \subsection{\DIFdel{Low-dimensional Embedding and Visualization}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend We aggregated calibration results into one to analyze the parameters in a collective way. It resulted in a $61\times20$ matrix (i.e., 31 observations in WT and 30 in MGAT1KO, and there are 20 calibration parameters). Then we applied t-distributed stochastic neighbor (t-SNE) embedding to encapsulate the high-dimensional data into three-dimensional space so that we could visualize each data point. \textbf{Figure~\ref{fig:embd_result}} presents the visualization of the t-SNE embedding of the calibration results. It shows not only clear differences between the two groups but also variances of cells (i.e., data points) within the groups and across the entire dataset.
\begin{figure}[!ht]
    \centering
