...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	ofstream result_file12((str+"FDUDB1_param.txt").c_str()  );
...
//Full FD UDB*******************************************************************************************************************************************
	if (Protocol ==1){
		...
		for(cycle_BCL = 1; cycle_BCL <2; cycle_BCL = cycle_BCL + 1){
			...
			if (cycle_BCL == 1){BCL_test = 1000; Hz = 1;}
			else if (cycle_BCL == 2){BCL_test = 500; Hz = 2;}
			else if (cycle_BCL == 3){BCL_test = 200; Hz = 5;}
			else if (cycle_BCL == 4){BCL_test = 100; Hz = 10;}
			else {BCL_test = 200;}
			...
			for (cycle = 0; cycle <=total_cycles; cycle = cycle + 1){		
				...
				for (tTemp = 0; tTemp <= interval; tTemp=tTemp+dt){ 
					...
					if (t<= waitTime) {dt = 0.01, Cell.V = V_o;}					//0.01
					else if (tTemp <= V_duration) {dt = 0.01, Cell.V = V_step;}	//0.005
					else {dt = 0.01, Cell.V = V_o;}								//0.01
