...
void WT_SCN5A_function(Cell_param *Cell_ptr, double t, double dt, double *Input_param);
...
void WT_SCN5A_function(Cell_param *Cell_ptr, double t, double dt, double *Input_param){
	...
	const double portion = 1/(1+ pow(10, (pH-pKa)) ); //Portion is of CHARGED DRUG
	//const double portion = 0; // Portion = 0 is for fully neutral flecainide
	...
	E_Na = (R*T/F)*log(Na_out/Na_in); //this is the natural log
...
//Rate Constants *************************************************************************************************************************************************************************
    ...
    //WT Fits Reduced Model (no IM1, IM2) --Final STM paper (Moreno, 2011):
...
//************************************ Drug Fitting *******************************************************************
    ...
    */
    ...
    /*
    //PRE-OP STIM INITIAL NEUTRAL RATES
    ax2 = 2.7926 * ax;
    a13n = 4.9550e-02 * a13;
    a_22 = 1.2915 * a2;
    b_33 = 2.2939 * b3;
    a_44 =  3.7496 * 3.122e-05 * a2;
    b_44 = 0.53413*0.10041 * a3;
   */
...
//			D_IM2_n = co_D_IM2 * ( D_IM2_o - hh2 * (a_55 * D_IM1  ) );
		...
		Cell_ptr->sum = Cell_ptr->O + Cell_ptr->OS + Cell_ptr->C1 + Cell_ptr->C2 + Cell_ptr->C3 + Cell_ptr->IC3 + Cell_ptr->IC2 + Cell_ptr->IF + Cell_ptr->IM1 + Cell_ptr->IM2 +
		...
		if (fabs (Cell_ptr->sum -1.0) > 0.0001) {cout << "Error in Wild Type Sum at t = " << t << ",\t sum = " << Cell_ptr->sum << endl;}
