function [ykto, ykslow1, ykslow2, ykur, ykss, yk1, yksum] = kcurrent_model3(p, hold_volt, volt, time_space, Ek)
    % default values
    ...
    % some parameters of ikslow1 are shared through ikslow2, ikur, and ikss
    % ikslow1: p1, p2, p3, p4, p5, p6, p7, p8
    % ikslow2: p1, p2, p3, p4, p5, p6, p7, p8
    % ikur: p1, p2, p3, p4, p5, p6, p7, p8
    ...
    % ikur
    % shared parameters for ikur
    shared_ikur_idx = 1:8;
    param_kur(shared_ikur_idx) = param_kslow1(shared_ikur_idx);

    % fixed parameters for ikur
    fixed_kur_idx = 9;
    param_kur(fixed_kur_idx) = kur0(fixed_kur_idx);

    % tuning parameters for ikur: p9, GKslow2
    tune_kur_idx = setdiff(1:11, [shared_ikur_idx, fixed_kur_idx]);
    param_kur(tune_kur_idx) = p(21:22);    
    ...
    ykur = ikur(param_kur, hold_volt, volt, time_space, Ek);
    ykss = ikss(param_kss, hold_volt, volt, time_space, Ek);
    yk1 = ik1(param_k1, hold_volt, volt, time_space, Ek);
    yksum = ykto + ykslow1 + ykslow2 + ykur + ykss + yk1;
end
