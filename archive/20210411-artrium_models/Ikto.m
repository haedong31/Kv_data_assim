function [current_trc] = Ikto(p, hold_volt, volt, time_space, Ek)
    ...
    f_eacv = p(17); % 0.2087704319 Ikto fraction phophorylated
    gmax = p(18); % 0.14067
    gmaxp = p(18)*(1.0-p(19)); % 0.14067*(1.0-0.387)
...
function [gv] = gating_variables(p, V)
    ...
    % for Ikto
    alpha1 = p(9).*exp(p(7).*(V+p(1))).*p(10);
    beta1 = p(11).*exp(-p(8).*(V+p(1))).*p(10);
    
    alpha2_temp1 = p(12).*exp((V+p(2))./(-1.0.*p(6)));
    alpha2_temp2 = p(13).*exp((V+p(2)+p(3))./(-1.0.*p(6)));
    alpha2 = p(15).*alpha2_temp1./(1.0+alpha2_temp2);

    beta2_temp1 = p(12).*exp((V+p(2)+p(3))./p(6));
    beta2_temp2 = p(14).*exp((V+p(2)+p(3))./p(6));
    beta2 = p(16).*beta2_temp1./(1.0+beta2_temp2);
    ...
    alpha1p = p(9).*exp(p(7).*(V+p(1)-p(4))).*p(10);
    beta1p = p(11).*exp(-p(8).*(V+p(1)-p(4))).*p(10);
    
    alpha2_temp1p = p(12).*exp((V+p(2)-p(5))./(-1.0.*p(6)));
    alpha2_temp2p = p(13).*exp((V+p(2)+p(3)-p(5))./(-1.0.*p(6)));
    alpha2p = p(15).*alpha2_temp1p./(1.0+alpha2_temp2p);
    
    beta2_temp1p = p(12).*exp((V+p(2)+p(3)-p(5))./p(6));
    beta2_temp2p = p(14).*exp((V+p(2)+p(3)-p(5))./p(6));
    beta2p = p(16).*beta2_temp1p./(1.0+beta2_temp2p);
