current_output <- function(time_space, current_trace) {
	...
	peak_time_idx <- which.max(current_trace)
	peak_time <- t[peak_time_idx]
	peak <- current_trace[peak_time_idx]

	check_pt1 <- any(is.nan(current_trace)) # not generated properly
	check_pt2 <- any(current_trace < 0) # negative current
	check_pt3 <- var(current_trace[1:holdt]) > 1e-3 # not stable at hold_volt
	...
	tau_idx <- which.min(abs(current_trace - tau_current))
	tau <- t[tau_idx]
	out[3] <- tau - holdt

	# output 4
	out45_jump_size <- floor((peak_time_idx + tau_idx)/3)
	out[4] <- current_trace[peak_time_idx + out45_jump_size]

	# output 5
	out[5] <- current_trace[peak_time_idx + out45_jump_size*2]
	
	# output 6
	last_idx <- length(t)
	last_current <- current_trace[last_idx]
	out[6] <- last_current

	# output 7
	o7_idx <- which.min(abs(current_trace[tau_idx:last_idx] - last_current))
...
current_output_ss <- function(time_space, current_trace) {
    # outputs for IKss
	...
	check_pt1 <- any(current_trace < 0) # negative current
	check_pt2 <- peak_time < holdt # can't generate current properly
	check_pt3 <- var(current_trace[1:holdt]) > 1e-5 # not stable at holding potential
