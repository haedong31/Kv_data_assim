...
% 
group_name = 'ko';
save_dir = strcat('calib_exp14_', group_name);
...
tune_idx_phase1{1} = [2, 6, 10, 13, 14];
...
tune_idx_phase2{1} = [15, 16, 17];
tune_idx_phase2{2} = [11, 12, 13];
tune_idx_phase2{3} = 3;
tune_idx_phase2{4} = 4;
...
tune_idx_phase3{1} = [1, 2, 6, 10, 13, 14, 15, 16, 17];
tune_idx_phase3{2} = [1, 2, 3, 4, 5, 8, 9, 11, 12, 13];
...
for l = 1:len_loop_idx
    ...
    % phase 1
    % 
%     obj_rmse(p01, 'tail', @kcurrent_model1, model_struct1, volt_space, time_space, yksum);
    opt_fun1 = @(p) obj_rmse(p, 'tail', @kcurrent_model1, model_struct1, volt_space, time_space, yksum);
...
%     obj_rmse(sol1, 'tail', @kcurrent_model1, model_struct1, volt_space, time_space, yksum)
...
%     obj_rmse(sol2, 'early', @kcurrent_model1, model_struct1, volt_space, time_space, yksum)
    ...
    for j = 1:num_currents
        ...
        switch model_struct3(j).name
            case 'ikto'
                kto_default = [33, 15.5, 20, 16, 8, 7, 0.03577, 0.06237, 0.18064, ...
    ...
    % phase 3
    %
    opt_fun3 = @(p) obj_rmse(p, 'all', @kcurrent_model1, model_struct3, volt_space, time_space, yksum);
    [sol3, fval] = fmincon(opt_fun3, p03, A, b, Aeq, beq, lb3, ub3, nonlcon, options);
    fprintf('[File %i/%i] %s Phase 3: %f \n', l, len_loop_idx, file_names{i}, fval)
    obj_rmse(sol3, 'all', @kcurrent_model1, model_struct3, volt_space, time_space, yksum)
