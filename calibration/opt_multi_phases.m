...
tune_idx_phase1{1} = 16;
tune_idx_phase1{2} = 11;
tune_idx_phase1{3} = 3;
tune_idx_phase1{4} = 4;
...
tune_idx_phase2{1} = [2, 4, 7, 10];
tune_idx_phase2{2} = [2, 5, 9];
tune_idx_phase2{3} = 1;
tune_idx_phase2{4} = 3;
...
[idx_info11, idx_info21, psize1] = gen_idx_struct(current_names, tune_idx_phase1);
...
model_info1 = [current_names; idx_info11; idx_info21];
...
[p0, lb1, ub1, max_param_len] = gen_optim_bound(model_struct1);
[~, lb2, ub2, ~] = gen_optim_bound(model_struct2);
[~, lb3, ub3, ~] = gen_optim_bound(model_struct3);
...
for l = 1:len_loop_idx
    ...
    % phase 1
    % 
    obj_rmse(p0, 'early', @kcurrent_model2, model_struct1, volt_space, time_space, yksum);
    opt_fun1 = @(p) obj_rmse(p, 'early', @kcurrent_model2, model_struct1, volt_space, time_space, yksum);
    [sol1, fval] = fmincon(opt_fun1, p0, A, b, Aeq, beq, lb1, ub1, nonlcon, options);
    fprintf('[File %i/%i] %s Phase 1: %f \n', l, len_loop_idx, file_names{i}, fval)
    obj_rmse(sol1, 'early', @kcurrent_model2, model_struct1, volt_space, time_space, yksum)

    %
    % phase 2
    %
    opt_fun2 = @(p) obj_rmse(p, 'tail', @kcurrent_model2, model_struct2, volt_space, time_space, yksum);
    [sol2, fval] = fmincon(opt_fun2, sol1, A, b, Aeq, beq, lb2, ub2, nonlcon, options);
    fprintf('[File %i/%i] %s Phase 2: %f \n', l, len_loop_idx, file_names{i}, fval)
    obj_rmse(sol2, 'tail', @kcurrent_model2, model_struct2, volt_space, time_space, yksum)

    %
    % phase 3
    %
    opt_fun3 = @(p) obj_rmse(p, 'all', @kcurrent_model2, model_struct3, volt_space, time_space, yksum);
    [sol3, fval] = fmincon(opt_fun3, sol2, A, b, Aeq, beq, lb3, ub3, nonlcon, options);
